...
export default {
    ...
    async execute(client, interaction) {
        ...
        //wczytywanie danych
        var allsnpsht = await db.get(`serverData/${interaction.guildId}/gc`)
        ...
        // Daj listę stacji
        var stationData = (await db.get(`stations/${$stacja}`)).val

        if (!stationData) {
            return interaction.editReply(`${customEmoticons.denided} Nie ma takiej stacji!`)
        }

        var serverData = Object.values((await db.get("serverData")).val || {})
